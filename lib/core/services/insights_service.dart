...
class InsightsService {
  ...
  static Future<List<RiskItemModel>> computeLocalRisk() async {
    try {
      ...
      return buildRisk(
        results[0] as List<AssetModel>,
        results[1] as List<WorkOrderModel>,
      );
    } catch (_) {
      return const [
        RiskItemModel(
  ...
  /// Pure risk scoring from already-fetched lists — no network.
  /// Dashboard calls this directly to avoid refetching assets/orders.
  static List<RiskItemModel> buildRisk(
      List<AssetModel> assets, List<WorkOrderModel> orders) {
